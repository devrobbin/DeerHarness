...
## 2026-09-11 · v1.7.3 · G4 运维收口：凭据轮换完成 + 轮换指引文档化

**变更摘要**
- **G4 完全闭环**：penguin 管理员密码已实际轮换——按上游 scrypt 参数（N=16384/r=8/p=1）生成新哈希直更 `web.db`，吊销全部 141 个历史会话，`.env` 同步更新；三项验证通过（新密码匹配 / 旧密码 `penguin-3983` 已失效 / DB 与写入值一致）。
- **docs/07 新增「上游凭据轮换」运维指引**：官方命令路径（penguin ≥0.2.6 `penguin server reset-admin-password`）+ 旧版直改 web.db 路径（生成 scrypt 哈希 → UPDATE + 吊销会话 → 三项验证 → 同步 .env），含实际执行记录。

**影响的文档**
- [07-安全与护栏](07-safety.md) v1.5.1
- [09-上手指南](09-guides.md)（troubleshooting 补轮换指引链接）
