...
export function UserAdmin() {
  ...
  const handleRotate = async (u: UserRow) => {
    if (!window.confirm(`重置「${u.username}」的 API Key？新密钥仅显示一次。`)) return;
    try {
  ...
  const handleDelete = async (u: UserRow) => {
    if (!window.confirm(`删除用户「${u.username}」？该操作不可恢复。`)) return;
    try {
  ...
  return (
    <div>
      <div className="mb-4 flex items-center justify-between">
        <h2 className="text-lg font-semibold dark:text-gray-100">用户管理</h2>
        <Button onClick={() => setShowCreate(!showCreate)} disabled={showCreate}>+ 创建用户</Button>
      </div>
      ...
      {showCreate && (
        <Card className="mb-6">
          <h3 className="mb-3 text-sm font-medium text-gray-700 dark:text-gray-200">创建用户</h3>
          <form onSubmit={handleCreate} className="grid grid-cols-3 gap-3">
            ...
            <div>
              <label className="mb-1 block text-xs text-gray-500 dark:text-gray-400">初始 API Key</label>
              <Input value={createForm.api_key} onChange={e => setCreateForm({ ...createForm, api_key: e.target.value })} required placeholder="请生成一个强密钥" />
            </div>
            <div>
              <label className="mb-1 block text-xs text-gray-500 dark:text-gray-400">角色</label>
      ...
      {rotatedKey && (
        <div className="mb-4 rounded border border-amber-300 bg-amber-50 p-3 dark:border-amber-700 dark:bg-amber-900/30">
          <p className="mb-1 text-xs font-medium text-amber-700 dark:text-amber-400">
            ⚠️ 「{rotatedKey.username}」的新 API Key（仅显示一次）
          </p>
          ...
          <Card key={u.id} className="flex items-center justify-between gap-3">
            <div className="min-w-0">
              ...
              <p className="mt-0.5 text-xs text-gray-500 dark:text-gray-400">
                {u.id} · 创建于 {new Date(u.created_at * 1000).toLocaleString()}
              </p>
            </div>
            <div className="flex shrink-0 items-center gap-2">
              ...
              <Button variant="ghost" onClick={() => handleRotate(u)}>重置密钥</Button>
              <Button variant="danger" onClick={() => handleDelete(u)}>删除</Button>
            </div>
          </Card>
        ))}
        {users.length === 0 && <p className="py-8 text-center text-gray-400 dark:text-gray-500">暂无用户</p>}
      </div>
    </div>
  );
}
